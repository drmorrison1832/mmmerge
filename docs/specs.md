...
> **Dernière mise à jour :** 2026-08-06 — v34
> **Résumé des derniers changements :** Nouvelle clé optionnelle `output_subfolder` (§3, §4) sur `gdocs`/`pdf`, utilisable **uniquement** avec `output_folder_id` (jamais avec `output_folder`, ni vide) : chemin dynamique (balises autorisées) résolu/créé **sous** `output_folder_id` plutôt que depuis la racine du Drive — comble un vrai manque, `output_folder`/`output_folder_id` étant jusqu'ici mutuellement exclusifs sans intermédiaire (dossier conteneur stable par ID, classement dynamique en dessous, ex: sous-dossiers mensuels). Même mécanisme côté `mail[]` : nouvelles clés `externalFolderId` (alternative à `externalFolder`, mutuellement exclusives) et `externalSubfolder` (utilisable uniquement avec `externalFolderId`), toujours strict (jamais soumis à `autoCreateFolders`), cohérent avec le comportement existant d'`externalFolder`. Voir architecture.md v36 pour le détail technique.
>
> **Résumé v33 (2026-08-06) :** Nouveau critère de filtre (§3) `not_equals`, négation exacte de `equals` (même comparaison insensible à la casse, sans normalisation des espaces) — permet par exemple `{ "label": "Type", "criterium": "not_equals", "value": "CDD" }` pour exclure une seule valeur sans lister toutes les autres via `match: "none"`. Se combine librement avec `equals` dans un même filtre. Voir architecture.md v35 pour le détail technique.
>
> **Résumé v32 (2026-08-06) :** Correction d'un bug réel (constaté en usage : plusieurs profils ciblant le même Sheet) où lancer un profil pouvait mettre à la corbeille un fichier gDocs/PDF généré par un **autre** profil. Cause : `mmm_outputs` est une cellule du Sheet, partagée par tous les profils qui le traitent, mais la purge en début de ligne (§2) traitait toute clé ne résolvant à aucune instance du profil en cours comme "orpheline" (instance retirée) et la purgeait — alors que la même situation se produit aussi, de façon indiscernable, quand la clé appartient simplement à un autre profil. La purge ne traite plus jamais une clé non résolue comme orpheline : elle est désormais toujours conservée telle quelle, jamais purgée. Contrepartie assumée : le nettoyage automatique d'une instance réellement retirée d'un profil doit maintenant se faire manuellement sur Drive. Voir architecture.md v34 pour le détail technique.
>
> **Résumé v31 (2026-08-06) :** Par défaut, `mmmerge` s'arrête désormais de lire le Sheet dès qu'il rencontre une ligne entièrement vide (toutes les colonnes vides) : cette ligne et tout ce qui suit n'est ni lu ni traité, avec un message indiquant où la lecture s'est arrêtée. Pratique pour laisser des brouillons/notes sous le tableau réel sans risquer qu'ils soient traités par erreur. Nouveau flag `--ignore-empty-rows` pour désactiver ce comportement et traiter le Sheet dans son intégralité comme avant. Voir architecture.md v33 pour le détail technique.
>
> **Résumé v30 (2026-08-06) :** `json2columns` copiait les nombres et booléens du fichier JSON source vers le Sheet sous forme de texte, ce qui empêchait Sheets de les reconnaître comme des nombres (l'écriture se fait sans analyse de texte, donc peu importe la locale du Sheet — le souci était que la valeur arrivait déjà en texte). Un nombre comme `123.45` ou un booléen `true` du fichier JSON est désormais écrit dans le Sheet avec son type d'origine (nombre/booléen natif, plus du texte), pendant que `{{ColonneCible}}` (templates, filtres) continue de recevoir une chaîne comme avant. Voir architecture.md v32 pour le détail technique.
>
> **Résumé v29 (2026-08-06) :** Réorganisation du dossier `configs/` (fait par l'utilisateur, profils/chemins adaptés en conséquence) : les profils d'exemple introduits en v28 vivent maintenant sous `configs/exemples/` (et non plus à la racine de `configs/`) ; les fichiers JSON compagnons de `json2columns` sous `configs/data/json2columns/` ; les corps HTML externes de `mail[].template_html_path` sous `configs/html/`. `loadConfig` accepte déjà un nom de profil contenant un sous-dossier (ex: `exemples/multiModuleExemple`) sans changement de code — le nom est simplement concaténé au chemin `configs/`. Les références internes (`json2columns[].file`, `mail[].template_html_path`) dans les profils déplacés ont été mises à jour vers leurs nouveaux emplacements. Voir architecture.md v31 pour le détail technique.
>
> **Résumé v28 (2026-08-06) :** Nouveaux profils d'exemple sous `configs/` (readme.md, section "Profils d'exemple prêts à l'emploi") : une paire Basic/Advanced par module (`gdocsExemple*`, `pdfExemple*`, `mailExemple*`, `columnsExemple*`, `json2columnsExemple*` — dix fichiers, chacun ciblant un seul module, `Advanced` avec plusieurs instances et la plupart des clés optionnelles) et `multiModuleExemple.json`, qui fait collaborer les cinq modules sur la même ligne (`json2columns` → `columns` → `gdocs`/`pdf` → `mail`, avec `attach: "generated"` et `{{link:...}}`). L'ancien `configs/exemple.json` (retiré, plus maintenu) est remplacé par `multiModuleExemple.json` comme profil de référence des tests. Voir architecture.md v30 pour le détail technique.
>
> **Résumé v27 (2026-08-06) :** Le module introduit en v26 sous le nom `lookup` a été renommé `json2columns` — nom plus explicite sur ce que fait le module (transformer un fichier JSON en colonnes du Sheet), avant toute adoption réelle en dehors de ce projet. Renommage pur, aucun changement de comportement ; l'entrée v26 ci-dessous a été mise à jour avec le nom final pour éviter toute confusion, plutôt que laissée telle quelle avec l'ancien nom. Voir architecture.md v29 pour le détail technique.
>
> **Résumé v26 (2026-08-06) :** Nouveau module `json2columns` (§2, §3), tableau d'instances comme `columns`/`gdocs`/`pdf`/`mail` : enrichit une ligne à partir d'un fichier JSON externe (`file`), en la retrouvant par la valeur d'une colonne du Sheet (`key_column`). S'exécute en tout premier, avant `columns[]` — colonnes importées immédiatement utilisables via `{{...}}` en aval. Contrairement à `columns[].output_column`/`link_column`, les colonnes cibles (les clés du fichier JSON) doivent déjà exister — `Erreur` explicite listant toutes les colonnes manquantes, rien n'est écrit pour la ligne si au moins une manque (délibérément différent : ces noms viennent d'un fichier externe, pas du profil que l'utilisateur contrôle). Clé sans correspondance dans le fichier → ligne ignorée pour cette instance avec un avertissement, pas une erreur. Valeurs non textuelles converties en chaîne ; valeurs non simples (objets imbriqués) rejetées au chargement du profil, avec la forme générale du fichier (existence, JSON valide, structure clé → objet). Fichier relu à chaque ligne, sans cache (même choix que `mail[].template_html_path`). Aucune vérification d'unicité de `key_column` entre lignes — jugée disproportionnée, deux lignes partageant la même valeur reçoivent le même contenu. Résumé de fin d'exécution (§5) gagne "Lignes enrichies via JSON". Voir architecture.md v28 pour le détail technique.
>
...
>
> **Résumé v23 (2026-08-02) :** Nouvelle clé `link_column` (§3), booléenne (défaut `false`), disponible sur `gdocs`/`pdf`/`mail` (pas `columns`) : écrit l'URL de sortie de l'instance dans une colonne dédiée du Sheet, en plus de `mmm_outputs` — nommée automatiquement `"<identifiant technique> output"` (ex: `"pdf[0] output"`), créée si absente (même mécanisme que `columns[].output_column`, toujours automatique). Le nom est dérivé de l'identifiant technique, jamais de `name` — `name` reste purement cosmétique (documenté comme tel ailleurs dans ce fichier) ; le baser sur une colonne persistée aurait donné un effet de bord réel à un simple renommage (nouvelle colonne créée, ancienne orpheline, puisque mmmerge ne supprime jamais de colonne). Écrit aussi en `--dry-run` (URL synthétique `(dry-run)`, comme `mmm_outputs`). Voir architecture.md v25 pour le détail technique. **Remplacé par v24 ci-dessus** (chaîne au lieu de booléen) avant toute adoption réelle.
>
...
**Lecture du tableau interrompue à la première ligne entièrement vide** : par défaut, `mmmerge` lit les lignes de données dans l'ordre et s'arrête dès qu'il en rencontre une dont **toutes** les colonnes sont vides (y compris `mmm_status`) — cette ligne et toutes celles qui suivent ne sont ni lues ni traitées, comme si le tableau de données s'arrêtait là. Un message console indique le numéro de la ligne où la lecture s'est arrêtée. Pratique pour garder des brouillons/notes sous le tableau réel sans risquer qu'ils soient traités par erreur. Le flag `--ignore-empty-rows` (§5) désactive ce comportement et traite le Sheet dans son intégralité — une ligne vide ainsi conservée reste éligible au traitement normal comme n'importe quelle autre ligne (son `mmm_status` vide suffit à la rendre éligible, voir §2), sans filtrage supplémentaire.
...
- Une ligne éligible déclenche une **ré-exécution intégrale et systématique de tout le pipeline depuis le début**. Avant toute nouvelle génération, chaque entrée `gdocs[i]`/`pdf[i]` déjà présente dans `mmm_outputs` est traitée selon l'instance correspondante dans le profil **actuel** :
  - Instance introuvable dans le profil **actuellement exécuté** → **ni purgée, ni perdue** : l'entrée est conservée telle quelle dans `mmm_outputs`, sans tenter de deviner pourquoi elle ne résout pas. `mmm_outputs` est une cellule du Sheet, pas un espace propre à un profil : si plusieurs profils traitent le même Sheet (cas réel, ex: un profil dédié à la génération gDocs et un autre au lookup JSON sur les mêmes lignes), une clé écrite par l'un est structurellement invisible pour l'autre — impossible de distinguer de façon fiable "cette instance a été retirée de ce profil" de "cette clé appartient à un autre profil qui partage ce Sheet". Ne jamais purger dans ce cas est le choix sûr par défaut (voir architecture.md, correction de ce point). Contrepartie assumée : une instance réellement retirée d'un profil (et qui ne sera plus jamais utilisée sous ce nom) ne voit plus son fichier nettoyé automatiquement — à faire manuellement sur Drive si besoin.
  ...
  - Une entrée `mail[i]` n'est **jamais** mise à la corbeille (voir ci-dessous), mais suit la même logique de conservation dans `mmm_outputs` : préservée si son instance est désactivée/filtrée pour cette ligne, ou introuvable dans le profil actuel ; abandonnée seulement si elle est active et va se régénérer cette ligne.
...
Chaque instance (Json2Columns, Columns, gDocs, PDF, ou Mail) accepte trois clés optionnelles :
...
- `conditions` (tableau non vide) : chaque condition compare la valeur d'une colonne (`label`) à une valeur attendue (`value`) selon un `criterium` — `equals` ou `not_equals` (sa négation exacte), tous deux insensibles à la casse (`"CDD"` et `"cdd"` sont équivalents) mais sans normalisation des espaces (`" CDD"` ne correspond pas à `"CDD"`).
...
### Lien de sortie dans une colonne (`link_column`) — gDocs, PDF, Mail

Une clé optionnelle supplémentaire, chaîne (nom de colonne, comme `columns[].output_column`), disponible sur `gdocs`/`pdf`/`mail` (mais pas `columns`, qui n'a pas de sortie de type "lien" — voir ci-dessous). Quand elle est renseignée, l'instance écrit son URL de sortie (en plus de `mmm_outputs`, sans le remplacer) dans la colonne du Sheet ainsi nommée, créée si elle n'existe pas encore dans l'en-tête — même mécanisme de création automatique que `columns[].output_column` (§3), toujours actif, sans flag à activer. Absente → rien n'est écrit.
...
### Enrichissement depuis un fichier JSON (`json2columns`, tableau d'instances)
...
- `attach` (`all` | `generated` | `external` | `none`) : détermine quelles pièces jointes sont utilisées, avec une correspondance stricte et symétrique entre le mode choisi et les listes remplies :
  ...
  - `external` : `external` doit être non vide (+ `externalFolder` ou `externalFolderId` requis), et `generated` doit être vide.
...
- `externalFolder` / `externalFolderId` (**exactement une** des deux, requise si `external` est utilisé) : dossier Drive dans lequel chercher les fichiers désignés par `external` — `externalFolder` (chemin avec balises, résolution dynamique identique à `output_folder`, voir architecture.md §7) ou `externalFolderId` (ID Drive littéral, comme `output_folder_id`). Toujours strict, jamais soumis à `autoCreateFolders`, y compris pour `externalSubfolder` ci-dessous.
- `externalSubfolder` (chaîne avec balises, optionnelle, utilisable **uniquement** avec `externalFolderId`) : sous-chemin dynamique résolu sous `externalFolderId` plutôt que depuis la racine — mêmes règles qu'`output_subfolder` (voir ci-dessous). Erreur de configuration si fournie avec `externalFolder`, ou si chaîne vide.
...
- **`gdocs`** (tableau) : `template_id`, **exactement une** des deux clés `output_folder` (chemin, balises autorisées, résolu dynamiquement — voir architecture.md §7) ou `output_folder_id`, plus optionnellement `output_subfolder` (chemin dynamique résolu **sous** `output_folder_id` — utilisable uniquement avec `output_folder_id`, jamais avec `output_folder` ; `Erreur` de configuration sinon, ou si chaîne vide), `output_filename` (chaîne, avec ou sans balises), `share` (optionnel, voir §3), `name`/`description`/`disable`/`template_link` (optionnels, voir §3).
- **`pdf`** (tableau, même structure que `gdocs`, sans `share`) : `template_id`, `output_folder` / `output_folder_id` (+ `output_subfolder` optionnel), `output_filename`, `name`/`description`/`disable`/`template_link`.
- **`mail`** (tableau) : voir §3 pour le détail des clés (`to`, `cc`, `subject`, `template_html`/`template_html_path`, `draft_only`, `attach`, `generated`, `externalFolder`/`externalFolderId` (+ `externalSubfolder` optionnel), `external`, `name`/`description`/`disable` — pas de `template_link`, qui n'a de sens que pour un template Drive externe).
...
- `--lines=4,14,15` : Restreint le traitement aux lignes spécifiées. Accepte aussi des plages (`début-fin`) mélangées à des numéros individuels — ex: `--lines=2,4-6,9` cible les lignes 2, 4, 5, 6 et 9. Une plage inversée (`6-4`, fin < début) lève une `Erreur` explicite, tout comme une ligne demandée qui n'existe pas dans le tableau de données (hors bornes, ou ligne 1 — l'en-tête) — avant même l'authentification, plutôt que d'être silencieusement ignorée.
...
- `--ignore-empty-rows` : Désactive l'arrêt automatique de lecture à la première ligne entièrement vide (voir §1) — le Sheet est lu et traité dans son intégralité, y compris les lignes situées après une ligne vide.
