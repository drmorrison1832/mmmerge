...
Une clé optionnelle supplémentaire, booléenne, défaut `false`, disponible sur `gdocs`/`pdf`/`mail` (mais pas `columns`, qui n'a pas de sortie de type "lien" — voir ci-dessous). Quand elle vaut `true`, l'instance écrit son URL de sortie (en plus de `mmm_outputs`, sans le remplacer) dans une colonne dédiée du Sheet, nommée automatiquement `"<identifiant technique> output"` (ex: `"pdf[0] output"`, `"mail[0] output"`), créée si elle n'existe pas encore dans l'en-tête — même mécanisme de création automatique que `columns[].output_column` (§3), toujours actif, sans flag à activer.

- Le nom de la colonne est dérivé de l'**identifiant technique** (`gdocs[0]`, `pdf[1]`...), jamais de `name` : `name` est documenté ailleurs dans ce fichier comme purement cosmétique, sans effet sur aucune référence technique — le baser sur une colonne persistée aurait fait d'un renommage anodin une opération avec un effet de bord réel (une nouvelle colonne apparaît, l'ancienne reste orpheline, puisque mmmerge ne supprime jamais de colonne).
